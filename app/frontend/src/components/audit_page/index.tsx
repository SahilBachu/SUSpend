...
function RiskBadge({ level }: { level: string }) {
  ...
  const cls = styles[normalized] ?? "bg-zinc-100 text-zinc-600 border border-zinc-200";
  return (
...
export default function AuditPage() {
  ...
  const { employee_name, audit_results, summary, high_risk_count, medium_risk_count, low_risk_count } = data;
  const valid_count = audit_results.length - (high_risk_count + medium_risk_count + low_risk_count);
  ...
  async function handleGenerateEmail() {
    ...
    setEmailError(null);
    try {
      const result = await api.generateEmail({ employee_name, audit_results, summary });
    ...
    <div className="min-h-screen bg-zinc-50">
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div>
            ...
            <h1 className="text-2xl font-bold text-zinc-900">
              Audit Report — <span className="text-indigo-600">{employee_name}</span>
            </h1>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { label: "High Risk", value: high_risk_count, color: "text-red-600 bg-red-50 border-red-100" },
            { label: "Medium Risk", value: medium_risk_count, color: "text-amber-600 bg-amber-50 border-amber-100" },
            { label: "Low Risk", value: low_risk_count, color: "text-green-600 bg-green-50 border-green-100" },
            { label: "Valid", value: valid_count > 0 ? valid_count : 0, color: "text-indigo-600 bg-indigo-50 border-indigo-100" },
          ].map((card) => (
            <div key={card.label} className={`rounded-2xl border px-5 py-4 ${card.color}`}>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-zinc-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-zinc-100 bg-zinc-50/50">
            <h2 className="text-base font-semibold text-zinc-900">Transactions</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-zinc-100">
              ...
              <tbody className="divide-y divide-zinc-50">
                {audit_results.map((r) => (
                  <tr key={r.transaction_id} className="hover:bg-zinc-50/50 transition-colors">
                    <td className="px-4 py-3 text-sm font-mono text-zinc-700 whitespace-nowrap">{r.transaction_id}</td>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-zinc-200 p-6">
          <h2 className="text-base font-semibold text-zinc-900 mb-4">Risk Distribution</h2>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
            <div className="px-6 py-5 border-b border-zinc-100 flex items-center justify-between">
              <h2 className="text-base font-semibold text-zinc-900">Generated Email</h2>
              <button
            ...
            <div className="px-6 py-5 overflow-y-auto flex-1 space-y-4">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-zinc-400 mb-1">Subject</p>
              ...
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-zinc-400 mb-1">Body</p>
